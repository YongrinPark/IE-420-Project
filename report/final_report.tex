...
\title{%
  ...
  \normalsize IE 420 – Industrial Engineering Systems Analysis\\
...
A Plotly/Dash interactive dashboard provides a real-time view of portfolio
value, VaR estimates, exceedances, and latency.
\end{abstract}
...
All market data are sourced from Yahoo Finance via the \texttt{yfinance}
Python library \citep{yfinance}.
...
\begin{itemize}
  ...
  \item \textbf{Intraday prices}: 1-minute OHLCV data for
        ...
        (approximately 391 bars per asset).
\end{itemize}
...
\begin{itemize}
  ...
  \item \texttt{yfinance} — market data
...
\IfFileExists{tables/backtest_table.tex}{%
  \input{tables/backtest_table.tex}%
}{%
  \begin{table}[H]
    \centering
...
\IfFileExists{tables/latency_table.tex}{%
  \input{tables/latency_table.tex}%
}{%
  \begin{table}[H]
    \centering
...
The parametric VaR approaches are orders of magnitude faster than Monte
Carlo simulation.  For a portfolio of $N = 5$ assets:
\begin{itemize}
  \item Parametric VaR requires one matrix-vector multiply —
        $\mathcal{O}(N^2)$ — and is essentially instantaneous ($<0.1$ ms).
  \item Monte Carlo VaR requires generating and evaluating $M$ correlated
        paths — $\mathcal{O}(MN)$ — scaling linearly in path count.
\end{itemize}
...
\begin{enumerate}
  ...
  \item Monte Carlo VaR scales linearly in path count; at $M = 10{,}000$
        paths it remains feasible ($\sim$X ms) but requires careful
...
integration with live market data streams.
